...
class SelfiePage extends React.Component {
  constructor() {
    ...
    this.startPosition = [0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875, 1];
    this.startPositionR = [0, 0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875];

    this.centerXOffset = width * 0.7;
    this.centerYOffset = -height * 0.25; //left and right
    this.animated = [];
    this.animatedR = [];
    this.currentName = "";
  ...
  translateStyle(index) {
    ...
    if (this.counterTurn) {
      ...
      let translateX = this.animatedR[index].interpolate({
        inputRange,
        outputRange
      });
      outputRange = outputRangeY;
      let translateY = this.animatedR[index].interpolate({
        inputRange,
        outputRange
      });
      return {
    ...
    outputRange = outputRangeX;
    let translateX = this.animated[index].interpolate({
      inputRange,
      outputRange
    });
    outputRange = outputRangeY;
    let translateY = this.animated[index].interpolate({
      inputRange,
      outputRange
    });
    return {
    ...
    if (this.counterTurn) {
      for (let xX in this.startPositionR) {
        ...
        Animated.timing(this.animatedR[xX], {
          toValue: this.startPositionR[xX] + 0.125,
          duration: 1000
        }).start();
        if (this.startPositionR[xX] >= 0.875) this.startPositionR[xX] = 0;
        else this.startPositionR[xX] += 0.125;
      }
    } else {
      for (let xX in this.startPosition) {
        ...
        Animated.timing(this.animated[xX], {
          toValue: this.startPosition[xX] - 0.125,
          duration: 1000
        }).start();
        if (this.startPosition[xX] <= 0.125) this.startPosition[xX] = 1;
        else this.startPosition[xX] -= 0.125;
      }
    }
  ...
  satUsers(translateStyles, styles, index) {
    return (
      ...
      >
        ...
        >
          <Hello
            user={this.generatedUsers[index]}
            image={this.generatedImages[index]}
          />
        </TouchableOpacity>
      </Animated.View>
    );
          ...
          >
            <ScrollView>
              ...
              {this.generatedUsers.map((user, index) => {
                if (index == 0)
                  return this.satUsers(
                    this.translateStyle(index),
                    styles.btn0,
                    index
                  );
                else if (index == 1)
