...
const CaptionList = props => (
    <ScrollView>
        {
            captionListData.map ((caption, key) => <Caption key={`${key}imageCaption`}>{caption}</Caption>)
        }

        <Text style={[styles.caption, {backgroundColor: '#4a4747', color: '#ffffff'}]}>(Write your own caption)</Text>
    </ScrollView>
);
