...
/*Process
  Step 1 : find the user data by email in the createAccount Colleciton and return that user gui
  Step 2 : check whether the user password is match to the database password
  Step 3 : check if there some data haven't filled yet. If Yes, return true, If No, return false
*/
exports.userProfileUndoneQuery = function(req, res) {
  MongoClient.connect(
    CONNECTION_URL,
    { useNewUrlParser: true },
    (error, client) => {
      ...
      callcreateAccountPromise().then(function(result) {
        if (result === null) {
          ...
          let callallCollectionsPromise = async () => {
            ...
            //Only query some fields, not all fields because user must fill in all data for that screen in order to update
            //That begin said, for that screen data, it would be either all filled or all empty
            let undone = false;
            if (
              aboutYouResult.zipCode === "" ||
              preferencesResult.interestedGender === "" ||
              interestsResult.likesArray === "" ||
              wouldYouRatherResult.s2r1 === "" ||
              localDestinationsResult.weekendLocation === ""
            ) {
              undone = true;
            }
